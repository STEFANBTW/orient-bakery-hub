...
const Deals: React.FC<DealsProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="font-display bg-[#f8f6f6] min-h-screen">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => onNavigate('Home')}>
             <span className="material-icons text-[#ff6a00] text-3xl group-hover:rotate-12 transition-transform">shopping_basket</span>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
             {links.map(link => (
                <button 
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium border border-gray-200 hover:bg-[#ff6a00] hover:text-white transition-colors whitespace-nowrap ${link.page === 'Deals' ? 'bg-[#ff6a00] text-white' : 'bg-white text-gray-600'}`}
                >
                  {link.label}
                </button>
             ))}
         ...
         <section className="relative bg-[#ef4343] overflow-hidden py-12 px-4">
            ...
            <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
               <div className="text-white max-w-2xl">
                  <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="bg-orange-500 px-3 py-1 rounded text-xs font-bold uppercase mb-4 inline-block shadow-md">Limited Time Offer</motion.span>
               ...
               >
                  ...
                  <div className="flex items-center gap-4 text-center">
                     ...
                     <span className="text-2xl font-bold text-gray-300 animate-pulse">:</span>
                     <div><div className="bg-gray-100 text-[#ef4343] font-black text-4xl w-16 h-16 rounded-lg flex items-center justify-center text-orange-500">55</div><span className="text-xs text-gray-400">SEC</span></div>
                  </div>
               </motion.div>
            </div>
         </section>
