...
const Process: React.FC = () => {
  ...
  // Handle Hash Navigation (Batch Verify)
  useEffect(() => {
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full h-full pointer-events-none">
            ...
            <div className="absolute bottom-12 right-4 md:right-8 md:bottom-24 w-full md:max-w-xl pointer-events-auto">
              {steps.map((step, index) => (
                <div
                  key={`card-${index}`}
                  className="absolute bottom-0 right-0 w-full glass-panel p-8 rounded-2xl border border-white/20 shadow-2xl transition-all duration-700 ease-out"
                  style={{
                    opacity: index === activeStep ? 1 : 0,
