...
const Produce: React.FC<ProduceProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="font-display bg-[#f6f8f6] min-h-screen text-slate-800">
         ...
         <div className="max-w-[1600px] mx-auto px-4 lg:px-6 h-16 flex items-center justify-between gap-8">
            <div className="flex items-center gap-2 flex-shrink-0 cursor-pointer" onClick={() => onNavigate('Home')}>
               ...
               <h1 className="text-2xl font-bold tracking-tight">
                  <span className="text-[#ff6a00]">ORIENT</span> <span className="text-slate-900">SUPERMARKET</span>
               </h1>
            </div>
            <div className="flex-1 max-w-2xl relative hidden md:block">
               <input 
                 type="text" 
                 className="w-full bg-slate-100 border-none rounded-lg py-3 pl-12 pr-4 text-sm focus:ring-2 focus:ring-[#ff6a00]" 
            ...
            <div className="flex items-center gap-6 flex-shrink-0">
               <button onClick={() => onNavigate('Favorites')} className="flex items-center gap-2 text-sm font-medium hover:text-[#ff6a00]"><span className="material-icons-outlined">favorite_border</span></button>
               <button onClick={() => onNavigate('Dashboard')} className="flex items-center gap-2 text-sm font-medium hover:text-[#ff6a00]"><span className="material-icons-outlined">person_outline</span></button>
         ...
         <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative w-full h-[300px] md:h-[400px] rounded-xl overflow-hidden shadow-lg group">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent flex items-center">
               <div className="pl-8 md:pl-16 max-w-lg">
                  <span className="inline-block bg-[#ff6a00] text-slate-900 text-xs font-bold px-3 py-1 rounded-full mb-4 uppercase tracking-wider">Farm to Table</span>
                  <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 leading-tight">Freshness Guaranteed. Every Day.</h1>
                  <button className="bg-[#ff6a00] hover:bg-white text-slate-900 font-bold py-3 px-8 rounded-lg transition-all duration-300 transform hover:translate-x-1 shadow-[0_0_20px_rgba(255,106,0,0.4)]">Shop Fresh Produce</button>
               </div>
            </div>
         </motion.div>
         )}
         ...
         <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <aside className="hidden lg:block lg:col-span-1 space-y-8">
               <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-4"><h3 className="font-bold">Quick Reorder</h3><span className="text-xs text-[#ff6a00] font-medium cursor-pointer">View All</span></div>
                     ...
                     {filteredReorder.length > 0 ? (
                        filteredReorder.map((item, i) => (
                           <div key={i} className="flex items-center gap-3 group cursor-pointer">
                              ...
                              <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate group-hover:text-[#ff6a00]">{item.name}</p><p className="text-xs text-slate-500">{item.price}</p></div>
                              <button className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-[#ff6a00] transition-colors"><span className="material-icons text-sm">add</span></button>
                           </div>
                        ))
                     ) : (
            ...
            <div className="col-span-1 lg:col-span-3 space-y-12">
               <section>
                  <div className="flex items-center justify-between mb-6">
                     ...
                     <a href="#" className="text-sm font-semibold text-[#ff6a00] hover:underline">See All</a>
                  </div>
                  <AnimatePresence>
                            ...
                            <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} whileHover={{ y: -5 }} key={item.name} className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden relative group">
                               ...
                               <div className="p-4">
                                  ...
                                  <div className="flex items-center justify-between">
                                     ...
                                     <button className="w-8 h-8 rounded-full bg-[#ff6a00] flex items-center justify-center hover:bg-[#e65e00]"><span className="material-icons text-sm font-bold text-white">add</span></button>
                                  </div>
                               </div>
                            </motion.div>
                         ))}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="col-span-1 md:col-span-2 lg:col-span-1 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl overflow-hidden relative group h-48 md:h-auto">
                       ...
                       <div className="relative p-6 h-full flex flex-col justify-end">
                          <span className="bg-[#ff6a00] text-slate-900 text-xs font-bold px-2 py-1 rounded w-fit mb-2">30% OFF</span>
                    ...
                    {filteredSpecials.map((item, i) => (
                      <motion.div layout whileHover={{ y: -5 }} key={item.name} className="bg-white border border-[#ff6a00] rounded-xl p-4 flex gap-4 hover:shadow-lg transition-all">
                         ...
                         <div className="flex flex-col justify-between flex-1">
                            ...
                            <div className="flex items-center justify-between">
                               <div className="flex flex-col"><span className="text-xs text-slate-400 line-through">${item.oldPrice}</span><span className="text-lg font-bold text-[#ff6a00]">${item.price}</span></div>
                               <button className="w-8 h-8 rounded-full bg-slate-100 hover:bg-[#ff6a00] flex items-center justify-center transition-colors"><span className="material-icons text-sm">add</span></button>
                            </div>
                         </div>
                      </motion.div>
                    ))}
