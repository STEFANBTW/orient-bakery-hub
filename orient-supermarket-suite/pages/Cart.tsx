...
const Cart: React.FC = () => {
  ...
  return (
    <div className="font-sans bg-[#f6f8f6] dark:bg-slate-950 min-h-screen flex flex-col transition-colors duration-300">
                      ...
                      <motion.div layout key={item.cartId} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col sm:flex-row gap-4 items-center">
                         ...
                         <div className="flex-grow min-w-0 w-full">
                            ...
                            <div className="flex justify-between items-end mt-2">
                               <div className="flex items-center border border-gray-200 dark:border-slate-600 rounded-lg bg-gray-50 dark:bg-slate-700">
                               ...
                               <div className="text-right">
                                  <span className="block text-xl font-bold text-slate-900 dark:text-white">${item.price.toFixed(2)}</span>
                               </div>
                            </div>
                         </div>
                      </motion.div>
                      ...
                      <motion.div layout key={item.cartId} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="bg-orange-50/50 dark:bg-orange-900/10 rounded-xl p-4 shadow-sm border border-orange-100 dark:border-orange-900/30 flex flex-col sm:flex-row gap-4 items-center">
                         ...
                         <div className="flex-grow min-w-0 w-full">
                            ...
                            <div className="flex justify-between items-end mt-2">
                               ...
                               <div className="text-right">
                                  <span className="block text-xl font-bold text-slate-900 dark:text-white">${(item.price * item.quantity).toFixed(2)}</span>
                                  <span className="text-xs text-gray-500 dark:text-gray-400">Unit: ${item.price.toFixed(2)}</span>
                               </div>
                            </div>
                         </div>
                      </motion.div>
             ...
             <div className="lg:col-span-4 space-y-6">
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-6 sticky top-24 transition-colors duration-300">
                   ...
                   <div className="space-y-3 mb-6">
                      <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Retail Subtotal</span><span>${cart.retailSubtotal.toFixed(2)}</span></div>
                      <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Wholesale Subtotal</span><span>${cart.wholesaleSubtotal.toFixed(2)}</span></div>
                      
                      {/* Dynamic Logistics Fee */}
                      {cart.wholesaleSubtotal > 0 && (
                          <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                              <span>Freight Logistics</span>
                              <span>${(cart.wholesaleSubtotal * 0.05).toFixed(2)}</span>
                          </div>
                      )}
