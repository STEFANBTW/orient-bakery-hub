...
const Aisles: React.FC<Props> = ({ onNavigate }) => {
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 p-6 md:p-12 transition-colors duration-300">
      <div className="max-w-[1600px] mx-auto">
        <motion.button 
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          onClick={() => onNavigate('Home')} 
          className="mb-8 text-[#ff6a00] flex items-center gap-2 font-bold hover:underline"
        >
        ...
        <motion.div 
          ...
          className="mb-10"
        >
           <h1 className="text-4xl font-black mb-2 text-slate-900 dark:text-white">Shop by Aisle</h1>
           <p className="text-gray-500 dark:text-gray-400">Explore our departments to find exactly what you need.</p>
        </motion.div>
        ...
        >
           {aisles.map((aisle) => (
              <motion.div 
                ...
                className="group relative h-64 bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-md cursor-pointer border border-gray-100 dark:border-slate-700"
              >
                 <div className="absolute inset-0 bg-gray-100 dark:bg-slate-700">
                    <img src={aisle.img} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={aisle.name} />
                 ...
                 <div className="absolute bottom-0 left-0 p-6 w-full">
                    <div className="flex justify-between items-end">
                      <div>
                        <motion.span 
                          className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-2 shadow-sm ${aisle.color}`}
                        >
                           {aisle.id.toUpperCase()}
                        </motion.span>
                        <h2 className="text-2xl font-bold text-white leading-tight group-hover:text-[#ff6a00] transition-colors">{aisle.name}</h2>
                      </div>
                      <motion.div 
                        initial={{ opacity: 0, x: -10 }}
                        whileHover={{ opacity: 1, x: 0 }}
                        className="bg-white text-[#ff6a00] rounded-full p-2"
                      >
