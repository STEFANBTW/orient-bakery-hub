...
export default function MarketCommandCenter() {
  const { role, skus, activeModule } = useRoles();
    ...
    <div className="h-full flex flex-col gap-6 bg-slate-50/50 p-2 rounded-3xl">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0 px-6 pb-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <Tabs value={activeModule || 'inventory'} className="w-full flex-1 flex flex-col">
            <TabsContent value="inventory" className="m-0 flex-1">
              <Card className="border-slate-200 shadow-sm flex flex-col h-full overflow-hidden bg-white rounded-xl">
                <div className="p-4 border-b bg-slate-50/50 flex items-center justify-between gap-4">
                  <div className="relative flex-1">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                    <Input placeholder="Search 5,000+ SKUs..." className="pl-9 h-9 text-xs bg-white" />
                  </div>
                  <Button variant="outline" size="sm" className="h-9 border-slate-200 text-[10px] uppercase font-bold">
                ...
                <ScrollArea className="flex-1">
                  <Table>
                    <TableHeader className="bg-slate-50 sticky top-0 z-10 border-b">
                      <TableRow>
                        <TableHead className="text-[9px] font-bold uppercase py-4">SKU / Brand</TableHead>
                        <TableHead className="text-[9px] font-bold uppercase text-right">Stock</TableHead>
                        <TableHead className="text-[9px] font-bold uppercase text-right">Price</TableHead>
                        <TableHead className="text-[9px] font-bold uppercase text-right">Action</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {skus.map(sku => (
                        <TableRow 
                          key={sku.id} 
                          className={`hover:bg-slate-50 cursor-pointer ${selectedSKU?.id === sku.id ? 'bg-slate-50' : ''}`}
                          onClick={() => setSelectedSKU(sku)}
                        >
                          <TableCell>
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-slate-900">{sku.name}</span>
                              <span className="text-[9px] text-slate-400 uppercase font-bold">{sku.sku} • {sku.brand}</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-right font-code text-xs">{sku.stock}</TableCell>
                          <TableCell className="text-right font-code text-xs font-bold">₦{sku.price.toLocaleString()}</TableCell>
                          <TableCell className="text-right">
                            <ActionGuard sensitivity="high" actionLabel="Edit" size="sm" variant="ghost" />
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
            ...
            <TabsContent value="ai" className="m-0 flex-1">
              <Card className="border-slate-200 shadow-sm p-8 space-y-8 bg-white rounded-xl h-full">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-6 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                    ...
                    {['Milk', 'Ribeye', 'Truffle Chips'].map(word => (
                      <div key={word} className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200">
                        <span className="text-xs font-bold">{word}</span>
                        <ArrowUpRight className="w-3 h-3 text-emerald-500" />
                      </div>
                    ))}
                  </div>
                  <div className="flex flex-col justify-center items-center text-center space-y-2 border border-dashed rounded-xl p-8">
            ...
            <TabsContent value="wholesale" className="m-0 flex-1">
              <Card className="border-slate-200 shadow-sm p-12 text-center space-y-6 bg-white rounded-xl h-full flex flex-col justify-center">
                 <Box className="w-12 h-12 text-slate-300 mx-auto" />
                 <h3 className="text-2xl font-bold uppercase tracking-tighter">Wholesale Kanban</h3>
                 <p className="text-sm text-slate-500 max-w-sm mx-auto">Manage corporate bulk orders and high-volume requests.</p>
