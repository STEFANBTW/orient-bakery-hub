...
export default function BakeryCommandCenter() {
  const { role, diningMenu, activeModule } = useRoles();
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <Tabs value={activeModule || 'production'} className="w-full flex-1 flex flex-col">
            ...
            <TabsContent value="catalog" className="m-0 flex-1">
              <Card className="border-slate-100 h-full p-12 text-center space-y-6 bg-white rounded-2xl">
                <Timer className="w-12 h-12 text-amber-600 mx-auto" />
                <h3 className="text-2xl font-bold tracking-tighter italic font-headline uppercase">Pastry Menu</h3>
                <p className="text-sm text-slate-500 max-w-sm mx-auto">Manage public-facing artisanal catalog and seasonal items.</p>
                <ActionGuard sensitivity="low" actionLabel="New Item" className="mx-auto bg-amber-600 hover:bg-amber-700 text-white" />
              </Card>
            </TabsContent>

            <TabsContent value="configurator" className="m-0 flex-1">
              <Card className="border-slate-100 h-full p-12 text-center space-y-6 bg-white rounded-2xl">
                <Palette className="w-12 h-12 text-amber-600 mx-auto" />
                <h3 className="text-2xl font-bold tracking-tighter italic font-headline uppercase">Custom Cake Orders</h3>
                <p className="text-sm text-slate-500 max-w-sm mx-auto">Review 3D custom cake designs and event specifications from clients.</p>
