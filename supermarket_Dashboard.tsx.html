...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="font-display bg-[#f8f7f5] dark:bg-slate-950 min-h-screen flex flex-col transition-colors duration-300">
               ...
               <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                     {[
                        ...
                        <motion.div variants={itemAnim} key={i} whileHover={{ y: -4, shadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1)" }} className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-700 relative group transition-all">
                           ...
                           <div className="flex items-center justify-between mt-auto">
                              ...
                              <motion.button whileTap={{ scale: 0.95 }} className={`p-2 rounded-lg text-xs font-semibold flex items-center gap-1 ${item.alert ? 'bg-[#ff6a00] text-white shadow-md' : 'bg-gray-50 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-[#ff6a00] dark:hover:bg-[#ff6a00] hover:text-white dark:hover:text-white transition-colors'}`}>
