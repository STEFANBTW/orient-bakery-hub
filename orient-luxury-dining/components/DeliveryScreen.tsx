...
const DeliveryScreen: React.FC = () => {
    return (
        <div className="bg-background-light dark:bg-background-dark min-h-screen text-gray-800 dark:text-gray-100 font-sans pb-24">
             {/* Map Hero */}
             <section className="relative w-full h-[85vh] overflow-hidden">
                <div className="absolute top-4 left-4 right-4 md:right-auto md:w-80 md:top-8 md:left-8 z-10 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md p-6 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 leading-tight">Live Delivery in Jos</h1>
                    <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">Real-time tracking of our fleet. We are currently experiencing higher than usual volume in Rayfield.</p>
                    <div className="flex items-center space-x-3 mb-4">
                        <span className="relative flex h-3 w-3">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-3 w-3 bg-primary"></span>
                        </span>
                        <span className="text-sm font-semibold text-primary">12 Bikes Active Now</span>
                    </div>
                    <div className="flex space-x-2">
                        <button className="flex-1 bg-primary hover:bg-primary/90 text-white font-medium py-3 px-4 rounded-lg shadow-md transition-all flex items-center justify-center space-x-2">
                            <span className="material-icons text-sm">restaurant_menu</span>
                            <span>Order Now</span>
                        </button>
                        <button className="p-3 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg transition-colors">
             ...
             <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20 pb-24">
                 ...
                 <section className="mb-24">
                     <div className="flex items-end justify-between mb-8">
                        <div>
                            <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">Quick Bundles</h2>
                            <p className="text-gray-500 dark:text-gray-400 text-lg">Curated packs for every occasion. Delivered in 30 mins.</p>
                        </div>
                        <a href="#" className="text-primary font-semibold hover:text-primary/80 flex items-center transition-colors">
                 ...
                 <section className="mb-24 grid grid-cols-1 lg:grid-cols-12 gap-12">
                    <div className="lg:col-span-4">
                        <div className="sticky top-24">
                            <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Track Your Order</h2>
                            <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-100 dark:border-gray-800">
                                    ...
                                    <span className="font-mono font-bold text-lg">#4092</span>
                                </div>
                                <div className="mb-6">
                                ...
                                <button className="w-full bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white font-medium py-3 rounded-lg transition-colors flex items-center justify-center">
                                    <span className="material-icons text-sm mr-2">support_agent</span> Contact Support
                                </button>
                            </div>
                        </div>
                    </div>
                    <div className="lg:col-span-8">
                         <div className="bg-white dark:bg-gray-900 rounded-xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm">
                            <div className="relative">
                                ...
                                <div className="absolute left-6 top-6 h-3/5 w-0.5 bg-primary"></div>
                 ...
                 <section>
                    <div className="text-center max-w-2xl mx-auto mb-12">
                        <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Delivery Zones & Fees</h2>
                        <p className="text-gray-500 dark:text-gray-400">Transparent pricing for every neighborhood in Jos. Fees go directly to supporting our riders.</p>
                    </div>
...
const TimelineItem = ({ icon, title, time, text, subText, active, completed }: any) => (
    <div className={`relative flex items-start mb-12 ${!active && !completed ? 'opacity-50' : ''}`}>
        <div className={`relative z-10 flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center border-4 ${completed ? 'bg-primary border-white dark:border-gray-900' : active ? 'bg-white dark:bg-gray-800 border-primary shadow-md' : 'bg-gray-200 dark:bg-gray-800 border-white dark:border-gray-900'}`}>
            <span className={`material-icons ${completed ? 'text-white' : active ? 'text-primary' : 'text-gray-400'} ${active && !completed ? 'animate-pulse' : ''}`}>{icon}</span>
        </div>
        <div className="ml-6 pt-1">
            <h4 className={`text-lg font-bold ${active ? 'text-primary' : 'text-gray-900 dark:text-white'}`}>{title}</h4>
            {text && <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{text}</p>}
            {subText && <span className="text-xs font-semibold text-primary mt-2 block">{subText}</span>}
            {time && <span className="text-xs text-gray-400 mt-2 block">{time}</span>}
        </div>
    </div>
);
...
const BundleCard = ({ title, price, desc, image, badge, badgeColor, people, time, icon }: any) => (
    <div className="group bg-white dark:bg-gray-900 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-800 cursor-pointer">
        <div className="relative h-64 overflow-hidden">
            ...
            {badge && (
                <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold shadow-sm uppercase tracking-wide ${badgeColor || 'bg-white/90 dark:bg-gray-800/90 text-primary'}`}>
                    {badge}
                </div>
            )}
        </div>
        <div className="p-6">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-4 line-clamp-2">{desc}</p>
            <div className="flex items-center justify-between">
                <div className="flex items-center text-gray-400 text-xs space-x-3">
                    <span className="flex items-center"><span className="material-icons text-sm mr-1">{icon || 'group'}</span> {people}</span>
                    <span className="flex items-center"><span className="material-icons text-sm mr-1">schedule</span> {time}</span>
                </div>
                <button className="bg-primary/10 hover:bg-primary hover:text-white text-primary font-semibold py-2 px-4 rounded-lg transition-colors text-sm">
                    Add to Cart
                </button>
            </div>
        </div>
...
const ZoneCard = ({ name, badge, badgeColor, fee, time, freeOver, popular }: any) => (
    <div className={`bg-white dark:bg-gray-900 rounded-lg p-6 border transition-colors relative overflow-hidden ${popular ? 'border-primary dark:border-primary/50 shadow-md' : 'border-gray-100 dark:border-gray-800 hover:border-primary/50'}`}>
        {popular && <div className="absolute top-0 right-0 bg-primary text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg">POPULAR</div>}
        <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-xl text-gray-900 dark:text-white">{name}</h3>
            {badge && <span className={`${badgeColor} text-xs font-bold px-2 py-1 rounded`}>{badge}</span>}
        </div>
