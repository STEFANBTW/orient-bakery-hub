       ...
       >
                   ...
                   {results.map((item, i) => (
                      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} key={i} className="flex items-start gap-4 p-3 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:shadow-md transition-shadow">
                         <div className="w-16 h-16 bg-gray-100 dark:bg-slate-700 rounded-md overflow-hidden shrink-0 relative">
                            ...
                            <div className="absolute bottom-0 right-0 bg-green-500 text-white p-[2px] rounded-tl-md"><span className="material-icons text-[10px] block">check</span></div>
                         </div>
                         <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-2 flex items-center justify-between">
                               <div className="flex items-center border border-gray-200 dark:border-slate-700 rounded-md bg-gray-50 dark:bg-slate-700 h-8">
                               ...
                               {item.status && <span className="text-xs text-green-600 dark:text-green-400 flex items-center gap-1 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded"><span className="material-icons text-[12px]">verified</span> {item.status}</span>}
                            </div>
                         </div>
                      </motion.div>
                   ))}
          ...
          <div className="px-6 py-4 border-t border-gray-200 dark:border-slate-800 shrink-0 flex items-center justify-between bg-white dark:bg-slate-900">
             ...
             <div className="flex gap-4">
                <button onClick={onClose} className="px-6 py-2.5 rounded-lg border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-slate-800">Cancel</button>
                <button 
                  onClick={processList}
                  disabled={processing}
